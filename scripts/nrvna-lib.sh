...
nrvna_start() {
    ...
    nrvna_status "$ws" && return 0
    mkdir -p "$ws"
    local log="${NRVNA_LOG_DIR%/}/nrvna-$(basename "$ws").log"
    "$(nrvna__bin)" "$model" "$ws" "$@" >"$log" 2>&1 &
    local launcher=$! waited=0
    while ! nrvna_status "$ws"; do
        kill -0 "$launcher" 2>/dev/null || { echo "nrvna-lib: daemon exited during startup (log: $log)" >&2; tail -5 "$log" >&2; return 1; }
        [ "$waited" -ge "$NRVNA_START_TIMEOUT" ] && { echo "nrvna-lib: startup timeout (log: $log)" >&2; return 1; }
